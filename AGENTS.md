...
- `crypto.randomUUID()` is **secure-context-only** — it's `undefined` on plain-HTTP origins that aren't `localhost` (e.g. a phone hitting a LAN IP), so a desktop `localhost` test can pass while a LAN client crashes. Use `randomUUID()` from `apps/web/src/uuid.ts` (falls back to `getRandomValues`, which is not context-gated) for client-generated IDs.
- nginx `docker/nginx.conf` serves the Vite shell with `expires -1` (revalidate) and `/assets/` at `max-age=1y` (immutable hashes). Don't add a `Cache-Control` via `add_header` inside those locations — it would reset the inherited security headers (CSP etc.).
